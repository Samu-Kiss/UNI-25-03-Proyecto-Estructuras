...
void print_graph_stdout(const Grafo& g,
    ...
    if (minr == INT_MAX) {
        LOG_ADVERTENCIA("Renderer", "Camino vacío: no hay nodos para renderizar la matriz.");
        return; // path vacío
    }
    ...
    for (size_t idx = 0; idx < path_nodes.size(); ++idx) path_index[path_nodes[idx]] = static_cast<int>(idx);

    // Dibujar solo el camino en forma de matriz (un caracter por celda, sin separadores)
    for (int r = minr; r <= maxr; ++r) {
        for (int c = minc; c <= maxc; ++c) {
            ...
            if (isPathNode) {
                ...
                if (!path_nodes.empty() && (id == path_nodes.front() || id == path_nodes.back())) {
                    ...
                    if (it != path_index.end()) {
                        ...
                        int next = (idx + 1 < (int)path_nodes.size()) ? path_nodes[idx + 1] : -1;
                        ...
                        int vprev_r = (prev == -1) ? 0 : (pr - r); // vector from curr to prev (row)
                        int vprev_c = (prev == -1) ? 0 : (pc - c); // vector from curr to prev (col)
                        int vnext_r = (next == -1) ? 0 : (nr - r); // vector from curr to next (row)
                        int vnext_c = (next == -1) ? 0 : (nc - c); // vector from curr to next (col)
                        ...
                        // Straight vertical (both neighbors in same column)
                        else if (vprev_c == 0 && vnext_c == 0) cellStr = "│";
                        else {
                            ...
                            if (down && right) cellStr = "┌";
                            else if (down && left) cellStr = "┐";
                            else if (up && right) cellStr = "└";
                            else if (up && left) cellStr = "┘";
                            else if (up && down) cellStr = "│";
                            else if (left && right) cellStr = "─";
                            else cellStr = "─";
                        }
                    }
                }
            ...
            // imprimir sin espacios entre celdas; aplicar color si es nodo del path y es letra (start/end)
            if (isPathNode && (id == path_nodes.front() || id == path_nodes.back())) {
                if (color) cout << termcolor::green << cellStr << termcolor::reset;
                else cout << cellStr;
            } else {
                // para los caracteres de trazado también aplicar color si requested
                if (isPathNode && color) cout << termcolor::green << cellStr << termcolor::reset;
                else cout << cellStr;
            }
        }
        cout << "\n";
