...
/*PROMPT: Analiza las salidas de consola y revisa su formato "\t[Modulo/Estado]:
 * ..." así como sus colores (Verde, Azul, Amarillo y Rojo). Una vez analizado
 * implementa un log helper que permita estandarizar el formato de salida que
 * analizaste.
 */
...
inline const char* LogEstadoTexto(LogEstado e) {
  switch (e) {
    case LogEstado::Exito:
      return "Exito";
    case LogEstado::Info:
      return "Info";
    case LogEstado::Advertencia:
      return "Advertencia";
    case LogEstado::Error:
      return "Error";
  }
  return "Info";
}
...
inline void Log(const std::string& modulo, LogEstado estado,
  ...
  os << "\t[" << modulo << "/" << LogEstadoTexto(estado) << "]: " << reset
     << mensaje << std::endl;
}
