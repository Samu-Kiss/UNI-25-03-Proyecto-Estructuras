...
bool valRutaMasCorta(string command) {
    ...
    if (cmd != "ruta_mas_corta") return false;
    ...
    if (y.empty() || y.find(' ') != -1) return false;
    if (!esEnteroPositivo(i) || !esEnteroPositivo(j) || !esEnteroPositivo(x) || !esEnteroPositivo(y)) return false;
...
bool valBaseRemota(string command) {
    ...
    if (e1 == -1) return false;
    string cmd = command.substr(0, e1);
    if (cmd != "base_remota") return false;
    ...
    if (j.empty() || j.find(' ') != -1) return false;
    if (!esEnteroPositivo(i) || !esEnteroPositivo(j)) return false;
