...
void ArbolHuffman::generarCodigosRecursivo(NodoHuffman* nodo,
  ...
  for (int i = 0; i < 256; ++i) {
    if (frecuencias[i] > 0) {
      NodoHuffman* nuevoNodo =
          new NodoHuffman(static_cast<char>(i), frecuencias[i]);
...
ArbolHuffman::~ArbolHuffman() { destruirRecursivo(this->raiz); }

void ArbolHuffman::imprimirCodigos() {
  std::cout << "--- Codigos de Huffman ---" << std::endl;
  for (size_t i = 0; i < this->caracteres.size(); ++i) {
    std::cout << "'" << this->caracteres[i] << "' : " << this->codigos[i]
              << std::endl;
  }
  std::cout << "--------------------------" << std::endl;
}
...
NodoHuffman* ArbolHuffman::obtenerRaiz() { return this->raiz; }